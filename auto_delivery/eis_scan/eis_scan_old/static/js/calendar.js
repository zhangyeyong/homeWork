...
Calendar=(function(){
	(function(){
		document.write('<link rel="stylesheet" href="/static/css/calendar.css"/>');
	})()
	...
	calendar.prototype={
		...
		setOption:function(config){
			...
			self.options={};
			self.festival='0000-01-01:元旦,2014-01-31:春节,2014-04-05:清明,0000-05-01:劳动,2014-06-02:端午,2014-09-08:中秋,0000-10-01:国庆,2014-10-02:重阳,2015-02-19:春节,2015-03-05:清明,2015-06-20:端午,2015-09-27:中秋,2015-10-21:重阳';
			for(k in defaults){
				// 检查自定义参数，若没有则使用默认参数
		...
		draw:function(n){
			...
			var currTime=self.month+n,
				...
				calendarWeek='<p class="week">'+tmpWeeks+'</p>',
				calendarDays='<div class="days">'+tmpDays+'</div>';
			var calendarCon=['<form class="calendarForm" name="calendarForm">',calendarTitle,calendarWeek,calendarDays,'</form>'].join('');
		...
		bindData:function(currTime,form){
			...
			for(var i = 0; i < tds.length; i++) {
				...
				if (dateArray[i]){
					...
					// 添加当前选择天
					if(currDateTime==selectedDayTime){
						$(tds[i]).addClass('current');
						tds[i].innerHTML=dateArray[i]+'<br/>'+self.options.selectedTip;
					}
					// 添加今天
			...
			// 点击日历控件选取日期
			daysCon.on('click',function(e){
				e.stopPropagation();
				if(_target && !e.target.className.indexOf('disabled')>-1){
					...
					var nextInput=$(This.options.nextInput),
						nextDate='',
						prevInput=$(This.options.prevInput);
					if(nextInput.length>0 && nextInput.className.indexOf('widget_timer')>-1){
						rangeDate=_target.value;
						nextDate=new Date(This.format2Day(_target.value,1));
						nextInput.value=nextDate.getFullYear()+'-'+This.formatDate(nextDate.getMonth()+1)+'-'+This.formatDate(nextDate.getDate());
						nextInput.click();
						rangeDate='';
					}else if(nextInput.length>0){
						nextInput.click();
					}else if(prevInput.length>0 && !prevInput.className.indexOf('widget_timer')>-1){
						prevInput.click();
					}
			...
			form.find('.monSelect').on('click',function(e){
				e.stopPropagation();
				var box=this.next(),df='';
				box.innerHTML='<div class="yearBox"></div><div class="monthBox"></div><div class="tc"><input type="button" value="确定"></div>';
				for(var i=self.options.beginYear;i<=self.options.endYear;i++){
					df+='<span>'+i+'</span>';
				}
				box.find('.yearBox').innerHTML=df;
				box.find('.monthBox').innerHTML='<span>1月</span><span>2月</span><span>3月</span><span>4月</span><span>5月</span><span>6月</span><span>7月</span><span>8月</span><span>9月</span><span>10月</span><span>11月</span><span>12月</span>'
		...
		setEnable:function(t){
			var self=this,
				dateValue=t.getAttribute('data-value'),
				dayCount=self.format2Day(dateValue);
			t=$(t);
			...
			if(typeof self.options.endDay=='number'){
				if(typeof self.options.startDay=='string' && self.options.startDay.indexOf('-')==-1){
					start=self.datemap[self.options.startDay];
					end=new Date().setDate(new Date().getDate()+self.options.endDay);
				...
				// 两个都是yyyy-mm-dd格式的日期
				if(self.options.startDay.indexOf('-')>-1 && self.options.endDay.indexOf('-')>-1){
					start=self.format2Day(self.options.startDay);
					end=self.format2Day(self.options.endDay);
				}
				// 开始日期是yyyy-mm-dd格式的日期
				else if(self.options.startDay.indexOf('-')>-1 && self.options.endDay.indexOf('-')==-1){
					start=self.format2Day(self.options.startDay);
			...
			// 设置下一个日历控件的值是当前控件值的第二天
			if(rangeDate!=='' && rangeDate.indexOf('-')>-1){
				start=self.format2Day(rangeDate,1);
			}
		...
		createCalendarBox:function(){
			if($('#calendarWarpper').length>0){
				$('#calendarWarpper').css({width:(_target.options.width+1)*_target.options.panels+'px'});
				return;
			}
			var self=this,
			...
			$(calendarWarpper).css({width:(self.options.width+1)*self.options.panels+'px'});
			calendarBox.id='calendarPanel';
			calendarBox=$(calendarBox);
			calendarBox.css({width:'100%',zIndex:2});
		...
		bindEvent:function(){
			var self=this;
			var preMon=self.panel.find('.preMon'),
				nextMon=self.panel.find('.nextMon'),
				preYear=self.panel.find('.preYear'),
				nextYear=self.panel.find('.nextYear');
			preMon.off('click');
			nextMon.off('click');
			preYear.off('click');
			nextYear.off('click');
			preMon.on('click',function(e){e.stopPropagation();self.goPrevMonth();});
			nextMon.on('click',function(e){e.stopPropagation();self.goNextMonth();});
			preYear.on('click',function(e){e.stopPropagation();self.goPreYear();});
			nextYear.on('click',function(e){e.stopPropagation();self.goNextYear();});
		...
		goPrevMonth:function(){
			var self=this;
			if (self.month <= self.options.beginYear*12){return;}
			self.month--;
			self.fillPanel();
			return this;
		},
		...
		goNextMonth:function(){
			var self=this;
			if (self.month >= self.options.endYear*12+11){return;}
			self.month++;
			self.fillPanel();
			return this;
		},
		...
		goPreYear:function(){
			var self=this;
			if(parseInt(self.month/12) <= self.options.beginYear) {return;}
			self.month-=12;
			self.fillPanel();
			return this;
		},
		...
		goNextYear:function(){
			var self=this;
			if(parseInt(self.month/12) >= self.options.endYear) {return;}
			self.month+=12;
			self.fillPanel();
			return this;
		},
		...
		show:function(target){
			var self=this;
			if(target.value!=target.options.tip && target.value.length>0){
				var tmpYear=parseInt(target.value.substring(0,4)),
					tmpMon=parseInt(target.value.substr(5,2))-1;
				self.month=tmpYear*12+tmpMon;
			}else{
				target.value='';
			}
			self.createCalendarBox();
			self.calendarWarpper.css({left:$(target).offset().left+target.options.revise[0] + "px",top:$(target).offset().top+$(target).height()+target.options.revise[1] + "px",display:'block'});
		...
		format2Day:function(date,n){
			...
			if(typeof n==='number'){
				return new Date().setDate(new Date(tmpDate[0],tmpDate[1]-1,tmpDate[2],23,59,59).getDate()+n);
			}else{
				return new Date(tmpDate[0],tmpDate[1]-1,tmpDate[2],23,59,59);
			}
	...
	return {
		show:function(target){
			...
			options= config==null ? arguments[1]==undefined ? {} :arguments[1] : eval("("+config+")");
			if(datepicker==null){
